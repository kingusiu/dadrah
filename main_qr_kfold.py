...
Parameters = recordtype('Parameters','vae_run_n, qr_run_n, qcd_sample_id, qcd_ext_sample_id, qcd_train_sample_id, qcd_test_sample_id, sig_sample_id, strategy_id, epochs, read_n, qr_model_t, poly_order, kfold_n')
...
qr_model_dir = '/eos/home-k/kiwoznia/data/QR_models/vae_run_'+str(vae_run_n)+'/qr_run_'+str(qr_run_n)
pathlib.Path(qr_model_dir).mkdir(parents=True, exist_ok=True)
envelope_dir = '/eos/user/k/kiwoznia/data/QR_results/analysis/vae_run_'+str(vae_run_n)+'/qr_run_'+str(qr_run_n)+'/sig_GtoWW35naReco/xsec_'+str(sig_xsec)+'/loss_rk5_05/envelope'
...
#****************************************#
paths = sf.SamplePathDirFactory(sdfr.path_dict).update_base_path({'$run$': 'run_'+str(params.vae_run_n)})

qcd_sample_all = dapr.merge_qcd_base_and_ext_datasets(params, paths, **cuts.signalregion_cuts)
...
for k, qcd_sample_part in zip(range(1,params.kfold_n+1), qcd_sample_parts):
    ...
    qcd_train, qcd_valid = js.split_jet_sample_train_test(qcd_sample_part, frac=0.7)
...
for k in range(1,params.kfold_n+1):
    ...
    mask[k] = False
    envelopped_cuts = calc_cut_envelope(cuts[mask,...])
    envelope_folds.update({'fold_{}'.format(k): envelopped_cuts.tolist()})

#+ 6th cut based on all folds for application to signal
envelope_folds.update({'fold_{}'.format(kfold+1): calc_cut_envelope(cuts).tolist()})
...
for k in range(1,params.kfold_n+2):
    ...
    polynomials = qrwf.fit_polynomial_from_envelope(envelope_json, [quantile], params.poly_order)
...
sig_sample = js.JetSample.from_input_dir(sig_sample_id, paths.sample_dir_path(sig_sample_id), **cuts.signalregion_cuts)
