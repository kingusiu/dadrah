...
def build_model(loss_fn, metric_fn, layers_n, nodes_n, lr_ini, wd_ini, activation, x_mu_std=(0.0, 1.0), initializer='he_uniform', opt_id='adam', regularizer=None):
    ...
    for i in range(layers_n):
        x = tf.keras.layers.Dense(nodes_n, kernel_initializer=initializer, kernel_regularizer=regularizer, activation=activation, name='dense_'+str(i+1))(x)

    outputs = tf.keras.layers.Dense(1, kernel_initializer=initializer, kernel_regularizer=regularizer, name='dense_out')(x)
...
def train_model(params, quantile, qcd_train, qcd_valid, score_strategy, k, tb_dir):
    ...
    model = build_model(quant_loss, ratio_metric, layers_n=params.layers_n, nodes_n=params.nodes_n, lr_ini=params.lr, wd_ini=wd_ini, 
        activation=activation, x_mu_std=x_mu_std, initializer=initializer, opt_id=params.optimizer, regularizer=regularizer)
    model.summary()
