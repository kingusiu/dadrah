...
# data inputs (selections): /eos/user/k/kiwoznia/data/QR_results/events/vae_run_$run_n_vae$/qr_run_$run_n_qr$/sig_GtoWW35naReco/xsec_$sig_xsec$/loss_rk5_05
# data outputs: /eos/user/k/kiwoznia/data/QR_results/analysis/vae_run_$run_n_vae$/qr_run_$run_n_qr$/sig_GtoWW35naReco/xsec_$sig_xsec$/loss_rk5_05/envelope

# set directories for saving and loading with extra envelope subdir for qr models
experiment = ex.Experiment(run_n=params.vae_run_n).setup(model_dir_qr=True, analysis_dir_qr=True)
...
#           for each xsec
#****************************************#
for sig_xsec in xsecs:

    # set up paths
    result_dir = '/eos/user/k/kiwoznia/data/QR_results/analysis/vae_run_{}/qr_run_{}/sig_{}/xsec_{}/loss_rk5_05/envelope'.format(params.vae_run_n, params.qr_run_n, signal_str, sig_xsec)
    ...
    for quantile in quantiles:
        ...
        for dat_train, dat_valid, model_n in zip(data_qcd_parts, data_qcd_parts[1:] + [data_qcd_parts[0]], list('ABCDE')):
            ...
            discriminator = qrwf.train_QR(quantile, dat_train, dat_valid, params, qr_model_t=params.qr_model_t)
    ...
    json_path = os.path.join(result_dir, 'cut_stats_allQ_'+ signal_str + '_xsec_' + str(sig_xsec) + '.json')
    print('writing envelope results to ' + json_path)
